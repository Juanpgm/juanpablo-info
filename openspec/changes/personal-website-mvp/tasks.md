...
- [x] 9.1 Lighthouse pass (Performance/A11y/Best Practices/SEO ≥95) across all locales; fix regressions. (Real `npx lighthouse` runs against `astro preview`, 7 URLs (es/en/de home + es blog index/post/projects/experience). Found and fixed 3 real a11y regressions: heading-order skip h1→h3 on /blog/ and /projects/ index pages — added `headingLevel` prop to BlogCard/ProjectCard; label-content-name-mismatch on footer mailto link — dropped mismatched `aria-label`; color-contrast 3.69:1/4.03:1 on `.badge-example`/`.badge-current` light-theme tint backgrounds — darkened to AA 4.5:1+. Also fixed hardcoded English `aria-label="Primary"` on Header nav (flagged in Phase 4/8, swept up here) via new `header.primaryNavLabel` i18n key. Final scores all 99-100 across all 4 categories × all 7 URLs.)
- [x] 9.2 Manual QA: non-Chromium plain-nav path + `prefers-reduced-motion` path (R5). (Verified by CODE AUDIT, not live browser emulation — no Playwright/Puppeteer/chrome-launcher installed as a project dependency, declined to add one. `prefers-reduced-motion`: confirmed `global.css`'s `*, *::before, *::after { transition-duration: 0.01ms !important; animation-duration: 0.01ms !important; }` guard unconditionally overrides all 6 `transition:` declarations + the sole `animation:` in the file via `!important`, plus explicit `::view-transition-*` and `[data-reveal]` resets; Astro's own bundled `viewtransitions.css` has a matching framework-level guard. Non-Chromium nav: corrected an inaccurate premise via reading Astro 7.2.0's actual `ClientRouter.astro`/`router.js` source — `<ClientRouter />`'s default `fallback="animate"` (not overridden anywhere in this codebase) makes it intercept same-origin `<a>` clicks via JS on ALL browsers, not just Chromium, doing a fetch+DOM-swap instead of native full navigation; this is NOT literal browser-default `<a href>` navigation as originally assumed. It still works correctly on non-Chromium — since the codebase declares zero `transition:animate`/`transition:name` directives, no fallback CSS animation is ever generated, so the swap is instant/unanimated there while Chromium gets the native View-Transition crossfade. Functionally equivalent outcome (working navigation) via a different mechanism than assumed.)
- [x] 9.3 Final commit.
